...
class Channel {
  ...
  constructor(options: any) {
    ...
    if (typeof options.participants === `string`) {
      ...
      const o = {};
      options.participants.forEach(
        (p) => (o[p] = Object.assign({}, DEFAULT_PERMISSIONS))
      );
      ...
      keys.forEach((k) => {
        if (k !== session.getKey().pub) {
          this.addParticipant(
            k,
            options.save,
            Object.assign({}, DEFAULT_PERMISSIONS, options.participants[k])
          );
        }
      });
      options.participants[session.getKey().pub] =
        options.participants[session.getKey().pub] ||
    ...
    this.onTheir(`participants`, (participants, k, from) => {
      ...
      for (let i = 0; i < keys.length; i++) {
        if (
          this.participants[keys[i]].admin ||
          this.participants[keys[i]].inviter
        ) {
          hasAdmin = true;
          break;
        }
      ...
      if (
        ...
        (this.participants[from].admin || this.participants[from].inviter)
      ) {
        if (typeof participants === `object`) {
          if (
            JSON.stringify(this.participants) === JSON.stringify(participants)
          ) {
            return;
          }
          ...
          Object.keys(participants).forEach((k) => {
            if (k !== session.getKey().pub) {
              this.addParticipant(
                k,
                true,
                Object.assign({}, DEFAULT_PERMISSIONS, participants[k]),
                true
              );
            }
          });
        ...
        if (sharedSecret && linkId) {
          ...
          publicState()
            ...
            .on(async (encrypted) => {
              ...
              const encryptedChatRequest = await Gun.SEA.encrypt(
                session.getKey().pub,
                sharedSecret
              ...
              util.gunAsAnotherUser(publicState(), sharedKey, (user) => {
                user
                  .get("chatRequests")
                  .get(channelRequestId.slice(0, 12))
  ...
  async getMySecretUuid() {
    if (!this.mySecretUuid) {
      const mySecret = await Gun.SEA.secret(
        session.getKey().epub,
        session.getKey()
      );
  ...
  async getSecret(pub) {
    if (!this.secrets[pub]) {
      const epub = await util.gunOnceDefined(
        publicState().user(pub).get(`epub`)
      );
  ...
  static async getChannels(callback, listenToChatLinks = true) {
    ...
    const handleChannel = async (value, ourSecretChannelId) => {
      if (value && !seen[ourSecretChannelId]) {
        ...
        const encryptedChatId = await util.gunOnceDefined(
          publicState().user().get(`chats`).get(ourSecretChannelId).get(`pub`)
        );
  ...
  async getLatestMsg(callback) {
    const callbackIfLatest = async (msg, info) => {
      if (!this.latest) {
        ...
        const t =
          typeof this.latest.time === `string`
            ? this.latest.time
            : this.latest.time.toISOString();
        if (t < msg.time) {
          this.latest = msg;
          callback(msg, info);
        }
    ...
    this.onMy("latestMsg", (msg) =>
      callbackIfLatest(msg, { selfAuthored: true, from: session.getKey().pub })
    );
    this.onTheir("latestMsg", (msg, k, from) =>
      callbackIfLatest(msg, { selfAuthored: false, from })
    );
  }
  ...
  async addParticipant(
    ...
    if (
      this.secrets[pub] &&
      JSON.stringify(this.secrets[pub]) === JSON.stringify(permissions)
    ) {
    ...
    if (save) {
      ...
      publicState()
        ...
        .get(ourSecretChannelId)
        .get(`pub`)
        .put(await Gun.SEA.encrypt({ pub }, mySecret));
    }
    if (this.uuid) {
  ...
  async send(msg) {
    if (typeof msg === `string`) {
      ...
      throw new TypeError(`msg param must be a string or an object`);
    }
  ...
  async onDirect(key: string, callback: Function, from?: string) {
    ...
    if (!from || (from !== `me` && from !== session.getKey().pub)) {
      this.onTheir(key, (val: any, k: string, pub: string) =>
        callback(val, pub)
      );
    }
  }
  ...
  async onMyDirect(key: string, callback: Function) {
    if (typeof callback !== "function") {
      throw new TypeError(
        `onMy callback must be a function, got ${typeof callback}`
      );
    }
    const keys = this.getCurrentParticipants();
    for (let i = 0; i < keys.length; i++) {
      ...
      publicState()
        ...
        .on(async (data: any) => {
          ...
          if (decrypted) {
            callback(
              typeof decrypted.v !== `undefined` ? decrypted.v : decrypted,
              key
            );
          }
        });
  ...
  async onTheirDirect(key: string, callback: Function, from: string) {
    // TODO: subscribe to new channel participants
    if (typeof callback !== "function") {
      throw new TypeError(
        `onTheir callback must be a function, got ${typeof callback}`
      );
    }
  ...
  hasWritePermission(pub) {
    return (
      !this.uuid ||
      (this.participants &&
        this.participants[pub] &&
        this.participants[pub].write)
    );
  }

  async _onTheirGroupFromUser(
    pub: string,
    key: string,
    callback: Function,
    subscription: any
    ...
    publicState()
      ...
      .get(theirSecretUuid)
      .get(key)
      .on(async (data: any, _a: any, _b: any, e: Event) => {
        if (subscription) {
        ...
        const decrypted = await Gun.SEA.decrypt(
          data,
          await this.getTheirGroupSecret(pub)
        );
        if (decrypted) {
          callback(
            typeof decrypted.v !== `undefined` ? decrypted.v : decrypted,
  ...
  setTyping(isTyping, timeout = 5) {
    ...
    this.put(
      `typing`,
      isTyping ? new Date().toISOString() : new Date(0).toISOString()
    );
    clearTimeout(this.setTypingTimeout);
    this.setTypingTimeout = setTimeout(
      () => this.put(`typing`, false),
      timeout
    );
  }
  ...
  getTyping(callback, timeout = 5) {
    ...
    this.onTheir(`typing`, (typing, key, pub) => {
      if (callback) {
        ...
        if (isTyping) {
          this.getTypingTimeouts[pub] = setTimeout(
            () => callback(false, pub),
            timeout
          );
        }
  ...
  getSimpleLink(urlRoot = "https://iris.to/") {
    if (this.uuid) {
      return `${urlRoot}?channelId=${this.uuid}&inviter=${
        session.getKey().pub
      }`;
    }
  ...
  async getChatLinks(opts: any = {}) {
    ...
    this.on("chatLinks", (links: any, from: string) => {
      ...
      Object.keys(links).forEach((linkId) => {
        ...
        if (chatLinks.includes(linkId)) {
          return;
        }
        const channels = [];
        chatLinks.push(linkId);
        const url = Channel.formatChatLink({
          urlRoot,
          inviter: from,
          channelId: this.uuid,
        ...
        if (subscribe) {
          publicState()
            .user(link.sharedKey.pub)
            .get("chatRequests")
            .map()
            .on(async (encPub, requestId, a, e) => {
  ...
  async createChatLink(urlRoot = "https://iris.to/") {
    ...
    const encryptedSharedKey = await Gun.SEA.encrypt(
      sharedKeyString,
      sharedSecret
    );
    ...
    const ownerEncryptedSharedKey = await Gun.SEA.encrypt(
      sharedKeyString,
      ownerSecret
    );
    ...
    userState()
      .get("chatLinks")
      .get(linkId)
      .put({ encryptedSharedKey, ownerEncryptedSharedKey });
  ...
  getChatBox() {
    ...
    const headerText = util.createElement(
      "div",
      "iris-chat-header-text",
      header
    );
    const onlineIndicator = util.createElement(
      "span",
    ...
    const typingIndicator = util.createElement(
      "div",
    ...
    const inputWrapper = util.createElement(
      "div",
    ...
    if (participants.length) {
      ...
      Channel.getActivity(publicState(), pub, (status) => {
        ...
        const undelivered = messages.querySelectorAll(
    ...
    this.getTheirMsgsLastSeenTime((time) => {
      ...
      unseen.forEach((indicator) => {
        ...
        if (msgEl.getAttribute("data-time") <= time) {
          ...
          if (!msgClass.includes("delivered")) {
    ...
    this.getMessages((msg, info) => {
      ...
      if (info.selfAuthored) {
        const cls =
          this.theirMsgsLastSeenTime >= msg.time
            ? "iris-seen yes"
            : "iris-seen";
      ...
      const msgEl = util.createElement(
        "div",
    ...
    textArea.addEventListener("keyup", (event) => {
      ...
      if (event.keyCode === 13) {
        ...
        if (event.shiftKey) {
          textArea.value = `${content.substring(
            0,
            caret - 1
  ...
  static setActivity(activity) {
    if (publicState().irisActivityStatus === activity) {
      return;
    }
    ...
    const update = () => {
      publicState()
        .user()
        .get(`activity`)
        .put({ status: activity, time: new Date(Gun.state()).toISOString() });
    };
    update();
  ...
  static getActivity(pubKey, callback) {
    ...
    publicState()
      ...
      .on((activity) => {
        ...
        const isActive =
          activityDate > new Date(now.getTime() - 10 * 1000) &&
          activityDate < new Date(now.getTime() + 30 * 1000);
        callback({
          isActive,
        ...
        if (isActive) {
          timeout = setTimeout(
            () => callback({ isOnline: false, lastActive: activity.time }),
            10000
          );
        }
      });
  ...
  static formatChatLink({
    ...
    if (channelId && inviter) {
      return `${urlRoot}?channelId=${enc(channelId)}&inviter=${enc(
        inviter
      )}&s=${enc(sharedSecret)}&k=${enc(linkId)}`;
    }
    return `${urlRoot}?chatWith=${enc(chatWith)}&s=${enc(sharedSecret)}&k=${enc(
      linkId
    )}`;
  }
  ...
  static async getMyChatLinks(
    ...
    user
      ...
      .map()
      .on((data, linkId) => {
        if (!data || chatLinks.includes(linkId)) {
          return;
        }
        const channels = [];
        user
          ...
          .on(async (enc) => {
            if (!enc || chatLinks.includes(linkId)) {
              return;
            }
            chatLinks.push(linkId);
            const sharedKey = await Gun.SEA.decrypt(enc, mySecret);
            const sharedSecret = await Gun.SEA.secret(
              sharedKey.epub,
              sharedKey
            );
            const url = Channel.formatChatLink({
