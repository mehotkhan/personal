...
export default function (groupName = "everyone") {
  return {
          ...
          if (isFollowing) {
            ...
            if (path && path !== "/") {
              node = path
                .split("/")
                .reduce(
                  (sum: any, s: string) => sum.get(decodeURIComponent(s)),
                  node
                );
            }
            callback(node, user);
    ...
    _cached_map(
      cached: Map<string, any> | undefined,
      cacheKey: string,
      path: string,
      myEvent: Event,
      callback: Function
      ...
      if (!cached) {
        ...
        this.get(path, (node: any, from: string) =>
          node.map((value: any, key: string, x: any) => {
            const item = { value, key, from };
            cached.set(key, item);
            for (const cb of callbacks.get(cacheKey).values()) {
    ...
    _cached_on(
      ...
      if (!cached) {
        ...
        cache.set(cacheKey, cached);
        this.get(path, (node: any, from: string) =>
          node.on((value: any, key: string, x: any) => {
            const item = { value, key, from };
    ...
    _cached_fn(fn: string, path: string, callback: Function) {
      ...
      const myEvent = {
        off: () => {
          const myCallbacks = callbacks.get(cacheKey);
