...
function notifyMsg(msg, info, channelId, onClick) {
  function shouldNotify() {
    ...
    if (
      channel.notificationSetting === 'mentions' &&
      !msg.text.includes(session.getMyName())
    ) {
      return false
    }
    return true
...
const publicVapidKey =
  'BMqSvZArOIdn7vGkYplSpkZ70-Qt8nhYbey26WVa3LF3SwzblSzm3n3HHycpNkAKVq7MCkrzFuTFs_en7Y_J2MI'
...
async function getNotificationText(notification) {
  const profile = await publicState()
  ...
  if (event === 'like') eventText = `${name} liked your post`
    ...
    eventText = `${name} mentioned you in their post`
...
function subscribeToIrisNotifications(onClick?: Function) {
  ...
  publicState()
    .user()
    .get('notificationsShownTime')
    .on((v) => (notificationsShownTime = v))
  ...
  group().on(
    `notifications/${session.getPubKey()}`,
    async (encryptedNotification, k, x, e, from) => {
      ...
      if (
        !notificationsShownTime ||
        notificationsShownTime < notification.time
      ) {
        console.log('was new!')
        const text = await getNotificationText(notification)
        const desktopNotification = new Notification(text, {
          icon: '/assets/img/icon128.png',
          body: text,
          silent: true,
        })
        desktopNotification.onclick = function () {
          const link = notification.target
            ? `/post/${notification.target}`
            : `/profile/${notification.from}`
...
async function sendWebPushNotification(recipient, notification) {
  ...
  const myKey = session.getKey()
  const shouldWebPush =
    recipient === myKey.pub || !(channel.activity && channel.activity.isActive)
...
export default {
  ...
  changeUnseenNotificationCount,
  subscribeToIrisNotifications,
  sendIrisNotification,
