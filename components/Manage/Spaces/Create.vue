...
const { createSpace } = useManage();

const SpaceDetails = reactive({
  title: "",
  about: "",
  relay: "wss://nostr.alizemani.ir",
  picture: "",
});
...
const selectPicture = async () => {
  if (pictureFile.value?.files && pictureFile.value?.files[0]) {
    ...
    SpaceDetails.picture = await file2Base64(resized);
  }
};
</script>
  ...
  <div class="fixed inset-0">
    <div class="flex min-h-full items-center justify-center p-4 text-center">
      ...
      >
        ...
        >
          <template #header> ایجاد فضای کاری جدید</template>
          ...
          <div class="form-group mb-6 border-b-1 pb-5">
            <div class="flex w-full justify-between gap-5">
              <UFormGroup label="عنوان" hint="اجباری" class="">
                <UInput
                  v-model="SpaceDetails.title"
                  size="md"
                  required
                  placeholder="یک عنوان بنویسید"
                  icon="i-heroicons-user-group"
              /></UFormGroup>
              <UFormGroup label="سرور توزیع کننده" hint="اجباری" class="">
                <UInput
                  v-model="SpaceDetails.relay"
                  size="md"
                  required
          ...
          <UFormGroup label="درباره" hint="اجباری" class="mb-6 border-b-1 pb-5">
            <UTextarea
              v-model="SpaceDetails.about"
              placeholder="درباره ای این فضای کاری چند کلمه بنویسید"
          /></UFormGroup>

          <ul class="mr-8 text-sm text-gray-500 list-disc text-right">
            <li
              :class="
                SpaceDetails.title?.length >= 3 &&
                SpaceDetails.about?.length >= 3
                  ? 'text-gray-500'
                  : 'text-red-500'
              "
            >
