...
const { createQa } = useManage();

const QuestionDetails = reactive({
  title: "",
  answer: "",
  order: 0,
});
  ...
  <div class="fixed inset-0">
    <div class="flex min-h-full items-center justify-center p-4 text-center">
      ...
      >
        ...
        >
          <template #header> ایجاد پرسش جدید </template>

          <div class="form-group mb-6 border-b-1 pb-5">
            <div class="flex w-full justify-between gap-3 items-center px-3">
              <UFormGroup label="پرسش" hint="اجباری" class="">
                <UInput
                  v-model="QuestionDetails.title"
                  class="w-[10rem]"
                  size="md"
                  required
              ...
              >
                <UInput v-model="QuestionDetails.order" class="ltr" />
              </UFormGroup>
            </div>
            <UFormGroup label="پاسخ" hint="اجباری" class="mt-3">
              <UTextarea
                v-model="QuestionDetails.answer"
                size="md"
                class="w-[10rem]"
                required
                placeholder="پاسخ به این پرسش"
          ...
          <ul class="mr-8 text-sm text-gray-500 list-disc text-right">
            <li
              :class="
                QuestionDetails.title?.length >= 5 &&
                QuestionDetails.answer?.length >= 5
                  ? 'text-gray-500'
                  : 'text-red-500'
              "
            >
