...
const DEFAULT_FOLLOW =
  'kOtd-O7c-3MODhHFqHNCws3QhUE_G68vaV6opRJ5qDY._-xxTTv08B5lvrbCZx8ulre9GDmHy3x2uPD3DJwpXAk'
...
export default {
  ...
  updateSearchIndex: _.throttle(
    () => {
      ...
      const values = Object.values(
        _.omit(searchableItems, Object.keys(blockedUsers()))
      )
  ...
  saveSearchResult: _.throttle(
    (k) => {
      local().get('contacts').get(k).put({
  ...
  getExtendedFollows(
    ...
    user(k)
      .get('follow')
      .map()
      .on((isFollowing: boolean, followedKey: string) => {
        // TODO: unfollow
        if (isFollowing) {
          this.addFollow(callback, followedKey, currentDepth, k)
          if (currentDepth < maxDepth) {
            this.taskQueue.push(() =>
              this.getExtendedFollows(
                callback,
                followedKey,
                maxDepth,
                currentDepth + 1
  ...
  updateNoFollowers: _.throttle(
    () => {
      const v = !(
        searchableItems[key.pub] && searchableItems[key.pub].followers.size > 0
      )
      if (v !== noFollowers) {
        noFollowers = v
  ...
  setOurOnlineStatus() {
    ...
    const setActive = _.debounce(() => {
      const chatId =
        activeRoute &&
        activeRoute.replace('#/profile/', '').replace('#/chat/', '')
    ...
    local()
      ...
      .then((settings) => {
        if (!settings) {
          ...
          settings.enableWebtorrent === undefined ||
          settings.autoplayWebtorrent === undefined
        ) {
          local()
            .get('settings')
            .get('enableWebtorrent')
            .put(DEFAULT_SETTINGS.local.enableWebtorrent)
          local()
            .get('settings')
            .get('autoplayWebtorrent')
            .put(DEFAULT_SETTINGS.local.autoplayWebtorrent)
        }
      })
    user()
      .get('block')
      .map()
      .on((isBlocked: boolean, user: string) => {
  ...
  getMyName() {
    return myName
  }, // TODO maybe remove and use iris.user().get('profile').get('name') instead?

  myPeerUrl: (ip: string) => `http://${ip}:8767/gun`,

  async shareMyPeerUrl(channel: Channel) {
    const myIp = await local()
      .get('settings')
      .get('electron')
      .get('publicIp')
      .once()
  ...
  addChannel(chat: Channel) {
    this.taskQueue.push(() => {
      const pub = chat.getId()
      if (this.channelIds.has(pub)) {
        return
      }
      ...
      if (chat.uuid) {
        ...
        chat.on('about', (v) =>
          local().get('channels').get(chat.uuid).get('about').put(v)
        )
        chat.getParticipants((participants) => {
          ...
          if (typeof participants === 'object') {
            ...
            keys.forEach((k, i) => {
              const hue = (360 / Math.max(keys.length, 2)) * i // TODO use css filter brightness
